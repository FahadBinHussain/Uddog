...
async function main() {
  ...
  const campaignData = [
    {
      ...
      description: 'We are raising funds to build a beautiful community garden in downtown Springfield. This garden will provide fresh produce for local families and create a green space for everyone to enjoy. Our goal is to create sustainable food sources while bringing the community together through gardening workshops and events.',
      goalAmount: 5000,
      category: 'community',
    ...
    },
    {
      ...
      description: 'Recent hurricanes have devastated our coastal communities, leaving thousands of families without homes, food, or basic necessities. This emergency relief fund will provide immediate assistance including temporary shelter, food supplies, medical aid, and help families rebuild their lives. Time is critical - these families need our help now.',
      goalAmount: 50000,
      category: 'emergency',
  ...
  for (const campaign of campaigns) {
    ...
    for (let i = 0; i < donationCount; i++) {
      ...
      const donationAmount = [10, 25, 50, 100, 250, 500][Math.floor(Math.random() * 6)]
      const isRecurring = Math.random() < 0.15 // 15% chance of recurring donation

      // Random date within the last 30 days
      const donationDate = new Date(Date.now() - Math.floor(Math.random() * 30 * 24 * 60 * 60 * 1000))
  ...
  for (const campaign of campaigns) {
    ...
    for (let i = 0; i < commentCount; i++) {
      const commenter = [...users, ...donors][Math.floor(Math.random() * (users.length + donors.length))]
      const commentText = commentTexts[Math.floor(Math.random() * commentTexts.length)]
      const commentDate = new Date(Date.now() - Math.floor(Math.random() * 25 * 24 * 60 * 60 * 1000))
  ...
  const impactStories = [
    {
      ...
      content: "We're thrilled to announce that we've reached 25% of our goal in just the first week! The support from the community has been overwhelming. We've already started purchasing seeds and tools for the garden."
    },
    {
      title: "Medical Update - Good News!",
      content: "Sarah's treatment is going well! The doctors are optimistic about her recovery. We're halfway to our goal and every donation is making a real difference in her treatment journey."
    },
    {
      title: "Animals Finding Homes",
      content: "Thanks to your support, we've been able to find homes for 15 animals this month! The shelter is still in need of funds, but we're making progress. Every donation helps us save more lives."
    },
    {
      title: "First Scholarships Awarded",
      content: "We're excited to announce that our first three scholarships have been awarded to deserving students! These bright minds will now have the opportunity to pursue their dreams in higher education."
    },
    {
      title: "Relief Supplies Distributed",
      content: "We've been able to distribute emergency supplies to over 200 families affected by the hurricane. Food, water, clothing, and temporary shelter have been provided. Thank you for making this possible!"
    }
  ]
  ...
  const campaignsToVerify = campaigns.slice(0, Math.floor(campaigns.length * 0.6))

  for (const campaign of campaignsToVerify) {
    await prisma.verification.create({
      data: {
        status: 'verified',
        verifiedAt: new Date(Date.now() - Math.floor(Math.random() * 20 * 24 * 60 * 60 * 1000)),
        campaign_id: campaign.campaign_id,
  ...
  console.log('Donors: donor1@example.com / donor1123 (or donor2, donor3, etc.)')
}
