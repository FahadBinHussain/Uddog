...
export async function POST(request: NextRequest) {
  try {
    ...
    // Log fraud report
    console.log(`Fraud report created for campaign ${campaignId} by user ${session.user.name}`)
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (status && Object.values(FRAUD_REPORT_STATUSES).includes(status)) {
      where.status = status
    }
...
export async function PATCH(request: NextRequest) {
  try {
    ...
    if (status === FRAUD_REPORT_STATUSES.RESOLVED && (!resolution || resolution.trim().length === 0)) {
      return NextResponse.json(
    ...
    const updatedReport = await prisma.fraudReport.update({
      ...
      data: {
        ...
        resolvedAt: status === FRAUD_REPORT_STATUSES.RESOLVED ? new Date() : null,
        resolvedBy: status === FRAUD_REPORT_STATUSES.RESOLVED ? parseInt(session.user.id) : null
      },
      include: {
        reporter: {
          select: {
    ...
    // If resolved and found to be fraudulent, pause the campaign
    if (status === FRAUD_REPORT_STATUSES.RESOLVED && resolution?.toLowerCase().includes('fraudulent')) {
    ...
    console.log(`Fraud report ${reportId} updated to ${status} by admin ${session.user.name}`)
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.log(`Fraud report ${reportId} deleted by admin ${session.user.name}`)
