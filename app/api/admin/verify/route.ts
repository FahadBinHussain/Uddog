...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      `Campaign ${campaignId} ${status} by admin ${session.user.name}`,
    );
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (pending) {
      ...
      status &&
      Object.values(VERIFICATION_STATUSES).includes(status as any)
    ) {
      where.verifications = {
        some: { status },
      };
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    console.log(
      `Verification ${verificationId} deleted by admin ${session.user.name}`,
    );
