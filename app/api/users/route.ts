  ...
  try {
    ...
    if (updates.newPassword) {
      ...
      if (session.user.role !== "admin") {
        ...
        if (
          !user ||
          !(await bcrypt.compare(updates.currentPassword, user.passwordHash))
        ) {
          return NextResponse.json(
  ...
  try {
    ...
    if (campaignsWithDonations.length > 0) {
      return NextResponse.json(
        {
          error:
            "Cannot delete user with active campaigns that have received donations",
        },
        { status: 400 },
      );
    }
