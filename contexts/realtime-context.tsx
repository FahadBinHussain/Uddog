...
export function RealtimeProvider({ children }: RealtimeProviderProps) {
  ...
  >("disconnected");
  const [subscribedCampaigns, setSubscribedCampaigns] = useState<Set<number>>(
    new Set(),
  );
  ...
  const subscribeToCampaign = useCallback(
    (campaignId: number) => {
      ...
      console.log(
        `Subscribing to real-time updates for campaign ${campaignId}`,
      );

      // Subscribe to donation updates
      const donationSubscription = subscribeToDonations(
        campaignId,
        (payload) => {
          ...
          window.dispatchEvent(
            new CustomEvent("donation-update", {
      ...
      const campaignSubscription = subscribeToCampaignUpdates(
        campaignId,
        (payload) => {
          ...
          if (payload.eventType === "UPDATE") {
            ...
            window.dispatchEvent(
              new CustomEvent("campaign-update", {
      ...
      const verificationSubscription = subscribeToVerificationUpdates(
        campaignId,
        (payload) => {
          ...
          if (
            ...
            window.dispatchEvent(
              new CustomEvent("verification-update", {
  ...
  const unsubscribeFromCampaign = useCallback(
    (campaignId: number) => {
      if (!subscribedCampaigns.has(campaignId)) return;

      console.log(
        `Unsubscribing from real-time updates for campaign ${campaignId}`,
      );
  ...
  const broadcastDonation = useCallback(
    async (campaignId: number, donation: any) => {
      try {
        await supabase.channel(`donations-${campaignId}`).send({
          type: "broadcast",
          event: "donation",
          payload: donation,
        });
